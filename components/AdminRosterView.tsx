...
const AdminRosterView: React.FC<AdminRosterViewProps> = ({ classes, registrations }) => {
    ...
    <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg border border-slate-200">
      ...
      <div className="space-y-6">
        {classes.map((classInfo) => {
          const registeredChildren = registrations[classInfo.id]?.filter(c => c.firstName.trim() || c.lastName.trim()) || [];
          return (
            <div key={classInfo.id} className="p-4 bg-slate-50 rounded-lg border border-slate-200">
              <h3 className="text-lg font-semibold text-indigo-800">{classInfo.name} ({registeredChildren.length} registered)</h3>
              {registeredChildren.length > 0 ? (
                ...
                <p className="mt-2 text-sm text-slate-500 italic">No children registered for this class yet.</p>
              )}
            </div>
          );
        })}
